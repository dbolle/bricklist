...
function GroupSectionHeader({ group, progress }) {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 bg-gray-50 dark:bg-gray-900 border-y border-gray-200 dark:border-gray-800 mt-1 first:mt-0">
      ...
      <span className={`text-xs font-semibold tabular-nums w-8 text-right ${pct === 100 ? 'text-green-600 dark:text-green-400' : 'text-blue-600 dark:text-blue-400'}`}>
        {pct}%
      </span>
    </div>
  )
...
export default function GroupPage() {
  ...
  if (error) {
    return (
      <div className="p-4">
        <div className="bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-900 rounded-xl p-4 text-sm text-red-700 dark:text-red-300">{error}</div>
      </div>
    )
  }
      ...
      <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 z-20 px-4 pt-4 pb-3 space-y-2">
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0 mr-2">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">{group.projects?.length || 0} projects</p>
          </div>
          <a
          ...
          <button onClick={handleDelete} className="flex-shrink-0 p-2 text-gray-400 dark:text-gray-500 hover:text-red-500">
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
          ...
          <div className="flex gap-1.5 flex-wrap">
            {group.projects.map((p) => (
              <Link
                key={p.id}
                to={`/projects/${p.id}`}
                className="px-2 py-0.5 bg-blue-50 dark:bg-blue-950 text-blue-700 dark:text-blue-300 rounded-full text-xs border border-blue-200 dark:border-blue-800 hover:bg-blue-100"
              >
                {p.name}
              </Link>
            ))}
