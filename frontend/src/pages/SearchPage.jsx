  ...
  return (
    <div className="flex items-center gap-3 bg-white rounded-xl border border-gray-200 p-3 shadow-sm">
      ...
      <div className="flex-1 min-w-0">
        <p className="font-semibold text-gray-900 text-sm truncate">{set.name}</p>
        <p className="text-xs text-gray-500">{set.set_num} · {set.year}</p>
        <p className="text-xs text-gray-400">{set.num_parts?.toLocaleString()} parts</p>
      </div>
      <button
        onClick={() => onAdd(set)}
    ...
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl">
        <div className="p-5">
          ...
          <p className="text-sm text-gray-500 mb-4">{set.name} ({set.set_num})</p>
...
export default function SearchPage() {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold text-gray-900 mb-4">Find a Set</h1>
      ...
      {!loading && results.length === 0 && query.length >= 2 && !error && (
        <div className="text-center py-8 text-gray-400 text-sm">No sets found for "{query}"</div>
      )}
